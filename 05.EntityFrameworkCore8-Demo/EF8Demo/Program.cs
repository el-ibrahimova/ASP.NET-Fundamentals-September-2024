...
async Task FilterByCollectionValue()
{
    ...
    // query with parameters => good practice is to use parameters
    var coll = await repo.AllReadonly<PrimitiveCollection>()
        .Where(pc => pc.Ints.Contains(number))
        .Where(pc => pc.Strings.Any(s => s == text))
        .ToListAsync();

    // The same as above but with literals => it is not good practice to write with literals => it is better to use parameters
